...
import {
  MIN_LOAN_DAYS,
  addDaysToDateInput,
  dateSummaryFromInput,
  minDateValue,
  validateLoanDateInputs,
} from '../utils/date'
...
type Props = {
  ...
  onSubmitLoan: (pickupLocal: string, dueLocal: string) => Promise<void>
}
...
export function LoanRequestDialog({ equipment, onDismiss, onSubmitLoan }: Props) {
  ...
  async function sendLoan() {
    ...
    setBusy(true)
    try {
      await onSubmitLoan(pickup, due)
      onDismiss()
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'Erro ao registrar solicitação'
      setErr(msg)
      if (/retirada|devolução|mínimo|mesmo dia|anterior/i.test(msg)) {
        setStep(1)
      }
    } finally {
      setBusy(false)
  ...
  const minDueLabel = pickup.trim() ? dateSummaryFromInput(addDaysToDateInput(pickup, MIN_LOAN_DAYS)) : null
          ...
          <>
            ...
            {step === 1 ? (
              <div className="loan-request-section stack">
                <p className="info-panel-title">Datas previstas de retirada e devolução</p>
                <p className="muted tiny loan-request-hint">
                  O período mínimo é de {MIN_LOAN_DAYS} dias entre retirada e devolução (não pode ser no mesmo dia). O
                  pedido ficará pendente até o administrador do NRDT aprovar.
                </p>
                <div className="field-datetime">
                  <label htmlFor="loan-request-pickup">Retirar em</label>
                  <input
                    id="loan-request-pickup"
                    type="date"
                    min={minPickup}
                    value={pickup}
                ...
                <div className="field-datetime">
                  <label htmlFor="loan-request-due">Devolver em</label>
                  <input
                    id="loan-request-due"
                    type="date"
                    min={minDue}
                  ...
                  />
                  {minDueLabel ? (
                    <p className="muted tiny field-hint" id="loan-request-due-hint">
                      Devolução no mínimo em <strong>{minDueLabel}</strong> ({MIN_LOAN_DAYS} dias após a retirada).
                    </p>
                  ) : (
                    <p className="muted tiny field-hint">Selecione primeiro a data de retirada.</p>
                  )}
                </div>
                {err ? (
              ...
              <div className="loan-request-section stack">
                <div className="loan-request-summary">
                  ...
                  <p className="loan-request-summary-line">
                    Retirada prevista: <strong>{dateSummaryFromInput(pickup)}</strong>
                  </p>
                  <p className="loan-request-summary-line">
                    Devolução prevista: <strong>{dateSummaryFromInput(due)}</strong>
                  </p>
                </div>
